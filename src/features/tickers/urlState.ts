...
export const SCREENER_DEFAULTS: ScreenerValues = {
  q: "",
  siMin: 0,
  dtcMin: 0,
  rvolMin: 0,
...
export function valuesFromParams(params: URLSearchParams): ScreenerValues {
  const raw = {
    q: params.get("q") ?? SCREENER_DEFAULTS.q,
    siMin: params.get("siMin") ?? String(SCREENER_DEFAULTS.siMin),
    dtcMin: params.get("dtcMin") ?? String(SCREENER_DEFAULTS.dtcMin),
    rvolMin: params.get("rvolMin") ?? String(SCREENER_DEFAULTS.rvolMin),
    catalyst: params.get("catalyst") === "1" ? "true" : "false",
    sort: (params.get("sort") ?? SCREENER_DEFAULTS.sort) as SortKey,
    dir: (params.get("dir") ?? SCREENER_DEFAULTS.dir) as Dir,
  };
